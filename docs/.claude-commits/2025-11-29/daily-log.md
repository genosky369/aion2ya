...
## 📊 오늘의 작업 요약
- 총 커밋 수: 3개
- 주요 작업: 마석/영석 계산기 구현 및 홈 화면 연동

## 🔥 주요 성과
1. 마석/영석 자동 각인 기댓값 계산기 완성
2. 확률 데이터 파일 구축 (엑셀 → TypeScript)
3. 박제 게시판 빌드 오류 수정
4. 홈 화면에서 계산기 접근 가능하도록 연동
...
### [02:25 KST] Docs: 2025-11-29 작업 로그 추가

**커밋 해시**: `8f6e10b`

#### 🎯 작업 내용
- 오늘의 작업 내용을 `.claude-commits/2025-11-29/daily-log.md`에 기록

---

### [02:30 KST] Fix: 홈 화면 마석/영석 계산기 링크 활성화

**커밋 해시**: `83cfb51`

#### 🎯 작업 내용
- 홈 화면의 마석/영석 시뮬레이터 카드에서 "준비중" 배지 제거
- 클릭 시 `/simulator/manastone` (계산기)로 이동하도록 링크 수정
- 설명 텍스트 변경: "마석 강화 시뮬레이터" → "자동 각인 기댓값 계산기"

#### 💡 의도 및 배경
- 사용자가 홈 화면에서 바로 마석/영석 계산기에 접근할 수 있도록 함
- 기존에는 "준비중"으로 표시되어 클릭 불가 상태였음

#### 🔧 기술적 변경사항

**수정된 파일:**
- `app/page.tsx` - SIMULATORS 배열에서 마석/영석 항목 수정
  - `available: false` → `available: true`
  - `href: '/simulator/magic-stone'` → `href: '/simulator/manastone'`
  - `description` 텍스트 변경
...
### 접근 경로
- **홈 화면**: 시뮬레이터 섹션의 "마석/영석" 카드 클릭
- **직접 URL**: `/simulator/manastone`

### 상단 네비게이션
- 홈 / 업데이트 내역 / 박제 게시판 / 커뮤니티 (4개)
- 시뮬레이터는 네비게이션에서 제거됨 (홈 화면에서만 접근 가능)

---

## 📋 다음 작업
- [x] 홈 화면에서 계산기 접근 가능하도록 연동
- [ ] 추가 계산기/시뮬레이터 구현 (영혼 각인 등)
