...
5. Create meeting-logs directory: {spec_base_path:.claude/specs}/{feature_name}/meeting-logs/
6. Call spec-pm agent to log the project kickoff (event_type: "phase_start", phase: "requirements")
...
**PM Logging**: After requirements are approved by the user, call spec-pm agent with:
- event_type: "phase_complete", phase: "requirements"
- session_context: Summary of all user inputs, decisions made, and the final requirements content
...
**PM Logging**: After design is approved by the user, call spec-pm agent with:
- event_type: "phase_complete", phase: "design"
- session_context: Summary of design decisions, architecture choices, and user feedback
...
**PM Logging**: After tasks are approved by the user, call spec-pm agent with:
- event_type: "phase_complete", phase: "tasks"
- session_context: Summary of task breakdown decisions and user feedback
...
| PM Meeting Log(auto)           | spec-pm(single call)                | .claude/specs/{feature_name}/meeting-logs/                   |
...
- event_type: "phase_start" | "phase_complete" | "user_feedback" | "decision_made" | "change_request" | "issue_raised" | "milestone"
...
**PM Auto-Logging Rules:**

- The main thread MUST call spec-pm agent automatically at the following points:
  1. **Phase Start**: When initializing a new feature (event_type: "phase_start")
  2. **Phase Complete**: When the user explicitly approves a document (event_type: "phase_complete")
  3. **User Feedback**: When the user provides feedback that leads to document changes (event_type: "user_feedback")
  4. **Change Request**: When the user requests modifications to an already-approved document (event_type: "change_request")
- The spec-pm agent call SHOULD be run in the background to not block the workflow
- The session_context passed to spec-pm MUST include: user's exact input, decisions made, documents affected, and any open issues
- When resuming a workflow (e.g., user returns to continue work), the main thread SHOULD call spec-pm with task_type: "context" to reload project context
