...
3. From the meeting logs, identify and internalize:
   ...
   - **Change history**: What was changed during requirements and design phases, and why
   - **Open issues**: Unresolved items that may affect task planning
   - **Context from previous phases**: Insights from requirements and design discussions that should inform task breakdown
...
3. Analyze all components that need to be implemented
4. Create tasks (incorporating insights from meeting logs - e.g., user-prioritized features first, known constraints)
5. Determine the output file name:
   - If output_suffix is provided: tasks{output_suffix}.md
   - Otherwise: tasks.md
6. Create task list
7. Return the result for review

### Refine/Update Existing Tasks (task_type: "update")

1. **Review meeting logs (Step 0 above)**
2. Read existing tasks document {tasks_file_path}
3. Analyze change requests {change_requests}
4. Based on changes (respecting decisions recorded in meeting logs):
   - Add new tasks
   - Modify existing task descriptions
   - Adjust task order
   - Remove unnecessary tasks
5. Maintain task numbering and hierarchy consistency
...
- The model MUST read and review all PM meeting logs BEFORE starting any task planning work
- The model MUST incorporate context from meeting logs (user decisions, priorities, open issues) into the task planning process
